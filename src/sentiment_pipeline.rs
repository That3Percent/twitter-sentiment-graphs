...
async fn get_sentiment_from_tweet(raw: String, out: Sender<Sentiment>, keywords: Keywords) {
	...
	let text = if let Ok(Tweet { text: Some(text) }) = tweet { text } else { return; };
	...
	if matching_keywords.is_empty() { return; }
...
pub async fn get_sentiments_from_tweets(mut tweets: impl Stream<Item=String> + Unpin, sentiments: Sender<Sentiment>, keywords: Keywords) {
	loop {
